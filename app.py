...
<style>
    ...
    .rank-num { font-size: 2rem; font-weight: 800; color: #d1d5db; margin-right: 1rem; }
    .cand-name { font-size: 1.15rem; font-weight: 600; }
    .section-label { font-size: 0.75rem; font-weight: 600; text-transform: uppercase;
                     letter-spacing: 0.05em; color: #9ca3af; margin-bottom: 0.3rem; }
...
if run:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        for uf in uploaded_files:
            ...
            pdf_files.append((path, uf.name))

        # Progress
        progress = st.progress(0, text="Starting screening...")
        results = []
        for i, (path, fname) in enumerate(pdf_files):
            progress.progress((i) / len(pdf_files),
                              text=f"Screening {fname}... ({i+1}/{len(pdf_files)})")
            from screener import extract_text_from_pdf, screen_resume
            text = extract_text_from_pdf(path)
            r = screen_resume(jd, text, fname)
            results.append(r)
...
if "results" in st.session_state and st.session_state["results"]:
    ...
    for i, r in enumerate(results):
        ...
        with st.expander(
            f"#{i+1}  {r.get('name','Unknown')}  —  Score: {score}/100  |  {rec}",
            expanded=(i < 3)
        ):
            ...
            with c1:
                ...
                st.markdown(f'<div class="section-label" style="margin-top:1rem;">Strengths</div>', unsafe_allow_html=True)
                for s in r.get("strengths", []):
                    st.markdown(f"✅ {s}")

                st.markdown(f'<div class="section-label" style="margin-top:1rem;">Gaps</div>', unsafe_allow_html=True)
                for g in r.get("gaps", []):
                    st.markdown(f"⚠️ {g}")

            with c2:
                st.markdown(f'<div class="section-label">Score</div>', unsafe_allow_html=True)
                st.markdown(
                    f'<div style="font-size:3rem;font-weight:800;color:{score_color};">{score}</div>'
                    f'<div style="color:#9ca3af;font-size:0.85rem;">out of 100</div>',
                    unsafe_allow_html=True
                )
                st.markdown(f'<div class="section-label" style="margin-top:1rem;">Recommendation</div>', unsafe_allow_html=True)
                st.markdown(f'<span class="score-badge {css}">{rec}</span>', unsafe_allow_html=True)
                st.markdown(f'<div class="section-label" style="margin-top:1rem;">Source File</div>', unsafe_allow_html=True)
